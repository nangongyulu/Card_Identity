...
def get_card_number_by_sobel(card_image):
    ...
    preprocess_image = util_.preprocess_card_image_by_sobel(card_image)
    ...
    for i in np.arange(len(contours)):
        ...
        if util_.verify_card_number_sizes(contour):
            # 完成旋转
            output_image = util_.rotate_card_image(contour, card_image)
            # 统一尺寸
            uniformed_image = util_.unify_card_image(output_image)
            # 追加到信用卡号区域的候选区域列表中
