...
ENGLISH_LABELS = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'J', 'K',
                  'L', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'U', 'V',
                  'W', 'X', 'Y', 'Z']
...
def predict_english(char_image, model):
    # 将原始图变成规定大小
    ...
    predict_english = np.argmax(results[0])
    return ENGLISH_LABELS[predict_english]

if __name__ == '__main__':
    char_image_file = 'data/enu_test/A/56.jpg'
    char_image = cv.imread(char_image_file, cv.COLOR_BGR2GRAY)
    # 以灰度图读取文件中的图片

    char = predict_english(char_image, load_english_model())
    print(char)
